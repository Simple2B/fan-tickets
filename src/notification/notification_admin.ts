...
document.addEventListener('DOMContentLoaded', () => {
    // Dispute buttons
    const disputesList = document.querySelectorAll('.dispute-tr-element') as NodeListOf<HTMLTableRowElement>;
    disputesList.forEach(dispute => dispute.addEventListener('click', () => {
    ...
    document.addEventListener("htmx:load", (e) => {
        ...
        if(target.classList.contains('notification-dispute-created')) {
            console.log('notification dispute created')
            target.addEventListener('click', (e) => {
                console.log(target.getAttribute('data-notification-payload'))
                const { room_uuid } = JSON.parse(target.getAttribute('data-notification-payload')) as NewDisputeNotificationPayload;
                createChatWindow();
